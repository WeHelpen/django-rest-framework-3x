...
from rest_framework_3x.renderers import HTMLFormRenderer
from rest_framework_3x.utils.urls import replace_query_param
...
def optional_login(request):
    ...
    """
    try:
        login_url = reverse('rest_framework_3x:login')
...
def optional_logout(request, user):
    ...
    """
    try:
        logout_url = reverse('rest_framework_3x:logout')
    except NoReverseMatch:
...
def format_value(value):
    ...
    if value is None or isinstance(value, bool):
        ...
        if any([isinstance(item, (list, dict)) for item in value]):
            template = loader.get_template('rest_framework_3x/admin/list_value.html')
        else:
            template = loader.get_template('rest_framework_3x/admin/simple_list_value.html')
        context = Context({'value': value})
        return template.render(context)
    elif isinstance(value, dict):
        template = loader.get_template('rest_framework_3x/admin/dict_value.html')
        context = Context({'value': value})
        return template.render(context)
